...
const CompleteBtnReceiving = ({ shipment = {}, onSuccess = () => {} }) => {
  ...
  const handleComplete = async () => {
    ...
    try {
      ...
      console.log('Receiving Data to be sent:', receivingData)

      // Create new record in receiving collection
      const createReceivingResponse = await api.post(`api/v1/receiving/add`, receivingData)
  ...
  return (
    <>
      <CButton
        ...
        variant="outline"
        onClick={handleOpenModal}
        disabled={loading || shipment?.warehouse_id === ''}
      >
            ...
            <div className="alert alert-info mt-3">
              <small>
                ...
                <ul className="mb-0">
                  ...
                  <li>Update warehouse status to &ldquo;In Warehouse&ldquo;</li>
                  <li>Set the final amount to {amount}</li>
                </ul>
              </small>
            </div>
